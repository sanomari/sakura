...
void CProfileManager::Add(int index, std::wstring_view name)
{
	// プロファイル検証
...
void CProfileManager::Rename(int index, std::wstring_view name)
{
	//旧プロファイル名取得
	if (index < 1 || index >= GetSize()) {
...
void CProfileManager::Delete(int index)
{
	if (index < 1 || index >= GetSize()) {
		throw std::out_of_range("index is out of range");
	}
	if (const auto d = GetDefault(); d.has_value()) {
		if (d.value() > index) {
			SetDefault(m_nDefaultIndex - 1);
		}
		if (d.value() == index) {
			ClearDefault();
		}
...
void CProfileManager::SetDefault(int index)
{
	if (index != LB_ERR && (index < 0 || index >= GetSize())) {
		throw std::out_of_range("index is out of range");
	}
	m_nDefaultIndex = index;
}
...
[[nodiscard]] std::wstring CProfileManager::GetAt(int index) const
{
